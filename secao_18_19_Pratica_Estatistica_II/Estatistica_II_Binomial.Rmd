---
title: "Estatística II"
author: "Abioluz Robson Behrend"
date: "2023-08-30"
output:
  html_document:
    df_print: paged
  pdf_document: default
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(reticulate)
use_virtualenv('/home/abioluz/Documentos/Cursos/Formacao_Cientista_Dados/venv')
```

```{python}
from scipy.stats import binom

```

# Distribuição Binomial

x = Eventos, 
p = Probabilidades
size = Eventos


Se jogar uma moeda 5 vezes, qual a prob. de dar cara 3 vezes?
```{r}
dbinom(x=3,size=5,prob=0.5)
```

```{python}
# binom.pmf(Eventos, Experimentos, Probabilidades)
binom.pmf(3,5,0.5)
```


Se eu passar 4 sinais de quatro tempos cada. Qual a probabilidade de eu pegar 0,1,2,3 e 4 sinais verdes?

```{r}
# Já sai acumulado
pbinom(q=4, size=4, prob=0.25)
```

```{python}
p = 0
for i in range(5):
  print(f'{i} -> {binom.pmf(i, 4, 0.25)}')
  p += binom.pmf(i, 4, 0.25)
print(p)

```

E se forem sinais de 2 tempos?


```{r}
dbinom(x=4, size=4, prob=0.5)
```

```{python}
binom.pmf(4, 4, 0.5)

```

Probabilidade Acumulada

```{python}
binom.cdf(4, 4, 0.5)

```


Se você fizer a prova de um concurso com 12 questões. “chutando” todas as questões, qual a probabilidade de acertar 7 questões? 
(4 alternativas cada questão)

```{r}
dbinom(x=7,size=12,prob=0.25)
```

```{python}
binom.pmf(7, 12, 0.25)

```






Se eu passar 4 sinais de quatro tempos cada. Qual a probabilidade de eu pegar 3 OU MAIS sinais verdes?

```{r}
pbinom(q=2, size=4, prob=0.25, lower.tail = F)
dbinom(x=3,size=4,prob=0.25) + dbinom(x=4,size=4,prob=0.25)
1 - pbinom(q=2, size=4, prob=0.25)
```



